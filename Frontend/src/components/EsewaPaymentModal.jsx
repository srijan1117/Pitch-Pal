...
export default function EsewaPaymentModal({ booking, registration, weeklyBooking, onClose }) {
  ...
  const handlePayment = async () => {
    ...
    try {
      const payload = {};
      if (isTourney) payload.registration_id = data.id;
      else if (isWeekly) payload.weekly_booking_id = data.id;
      else payload.booking_id = data.id;

      const result = await initiateEsewaPayment(payload);
      ...
      const errorMsg = err.response?.data?.ErrorMessage || err.response?.data?.detail || "An error occurred while connecting to eSewa. Please check your connection.";
      ...
      <div className="bg-white w-full max-w-md rounded-[2rem] overflow-hidden shadow-2xl shadow-black/20 animate-in zoom-in-95 duration-300">
        ...
        <div className="p-8">
          <div className="mb-8 text-center">
            ...
            <p className="text-gray-500 text-sm">You are about to pay through eSewa for your {isTourney ? "tournament participation" : (isWeekly ? "weekly recurring booking" : "booking")}.</p>
          </div>

          <div className="bg-gray-50 rounded-2xl p-6 mb-8 space-y-4">
            <div className="flex items-start gap-4">
              <div className="w-10 h-10 rounded-xl bg-white flex items-center justify-center shadow-sm shrink-0">
                {isTourney ? <Ticket className="text-[#60bb46] w-5 h-5" /> : (isWeekly ? <Calendar className="text-[#60bb46] w-5 h-5" /> : <Calendar className="text-[#60bb46] w-5 h-5" />)}
              </div>
              <div>
            ...
            <div className="flex items-start gap-4">
              <div className="w-10 h-10 rounded-xl bg-white flex items-center justify-center shadow-sm shrink-0">
                <MapPin className="text-[#60bb46] w-5 h-5" />
              </div>
              <div>
                <p className="text-xs text-gray-500 font-medium uppercase tracking-wider mb-0.5">Location</p>
                <p className="font-bold text-gray-900 line-clamp-1">
                  {isTourney 
                    ? (data.tournament_detail?.location || data.tournament?.location || "N/A")
                    : (data.court_address || data.court_detail?.address || "N/A")}
                </p>
              </div>
            </div>
            ...
            <div className="pt-4 border-t border-gray-200 flex justify-between items-center">
              ...
              <span className="text-2xl font-black text-[#60bb46]">{displayAmount}</span>
            </div>
          </div>
          ...
          <button
            ...
            disabled={loading}
            className="w-full h-16 bg-[#60bb46] hover:bg-[#52a13c] disabled:bg-gray-300 text-white rounded-2xl font-bold text-lg transition-all shadow-lg shadow-green-100 flex items-center justify-center gap-3 active:scale-[0.98]"
          >
            {loading ? (
              <div className="w-6 h-6 border-3 border-white/30 border-t-white rounded-full animate-spin"></div>
            ) : (
              <>
                <span>Pay via eSewa Gateway</span>
              </>
            )}
          </button>
